...
def run_regression_with_caching(
    ...
    *,
    ...
    vcov_type: Literal[
        "kabupaten_full_code", "kecamatan_full_code", "province_full_code", "None"
    ] = "kabupaten_full_code",
    ...
    if vcov_type == "None":
        model = pf.feols(spec.formula, data=df)
    else:
        model = pf.feols(spec.formula, data=df, vcov={"CRV1": vcov_type})

    result = cached_result_from_model(model)
...
def cached_result_from_model(model: Any) -> CachedRegressionResult:
    ...
    return CachedRegressionResult(
        ...
        vcov=pd.DataFrame(
            model._vcov, index=coefficients.index, columns=coefficients.index
        ),
