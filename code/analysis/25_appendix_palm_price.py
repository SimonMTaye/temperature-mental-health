...
WB_PINK_SHEET_URL = "https://thedocs.worldbank.org/en/doc/5d903e848db1d1b83e0ec8f744e55570-0350012021/related/CMO-Historical-Data-Monthly.xlsx"
    ...
    try:
        ...
        for r in range(1, 12):
            ...
            for i, v in enumerate(row):
                ...
                if (
                    isinstance(v, str)
                    and v.strip().lower() in ("period", "date")
                    and period_col is None
                ):
                    period_col = i + 1
        ...
        print(
            f"  downloaded {len(df)} months from WB Pink Sheet ({df.date.min().date()} -> {df.date.max().date()})."
        )
        return df
    except Exception as exc:
...
def load_inline() -> pd.DataFrame:
    rows = [
        (pd.Timestamp(year=y, month=m, day=15), p)
...
def main() -> None:
    ...
    ax.axvspan(
        pd.Timestamp("2007-07-06"),
        pd.Timestamp("2008-08-18"),
        color="#cfe3f4",
        alpha=0.55,
        label="IFLS4 fielding (Jul 2007 -- Aug 2008)",
    )
    ax.axvspan(
        pd.Timestamp("2014-09-06"),
        pd.Timestamp("2015-12-18"),
        color="#fbe1cf",
        alpha=0.55,
    ...
    ax.set_title(
        "Palm-oil price across the inter-IFLS period and both fielding windows, 2007--2015",
        fontsize=12.5,
        weight="bold",
        loc="left",
        pad=8,
